...
for i = 1:size(configParamsList,1)
    ...
    thisAttributes = erase(thisAttributes, {'isfile', 'isfolder', 'isfunction'});
    ...
    % Validate additional attributes of paramters
    if parameterParamFlag
        if contains(paramProps.attributes(indx), 'isfunction')
            if ~ismember(exist(thisParamVal, 'file'), [2 6])
                configError{i,2} = ['Function is not', ...
                    ' on the search path or in the current folder.'];
            end
        end
    end
    
    % Validate additional attributes of files that are not generated by the
    % pipeline (isfile and isfolder attributes)
